  ...
  @Override public void onActivityResult(int requestCode, int resultCode, Intent data) {
    if (requestCode == ImageCaptureManager.REQUEST_TAKE_PHOTO && resultCode == RESULT_OK) {
      ...
      if (directories.size() > 0) {
        ...
        originalPhotos.add(path);
        Message message = new Message();
        message.what=0x123;
        PhotoPickerActivity pickerActivity = (PhotoPickerActivity) getActivity();
        pickerActivity.mHandler.sendMessage(message);

//        PhotoDirectory directory = directories.get(INDEX_ALL_PHOTOS);
//        directory.getPhotos().add(INDEX_ALL_PHOTOS,photo);
//        directory.setCoverPath(path);
//        photoGridAdapter.setAddPhoto(photo);
        photoGridAdapter.notifyDataSetChanged();
